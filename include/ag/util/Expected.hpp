...
public:
    ...
    constexpr expected() : has_value_(true) { new (&value_) T(); }

    constexpr expected(const T& value) : has_value_(true) { new (&value_) T(value); }

    constexpr expected(T&& value) : has_value_(true) { new (&value_) T(std::move(value)); }
